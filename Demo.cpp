...
int main() {
  ...
  cout<<"address from demo "<<p1.address<<endl;
   Game game(p1, p2);
   cout<<" p1 address from demo is: "<<&p1<<"and selfPointer->"<<p1.selfPointer<<endl; 
   cout<<"dealCards: Player1("<<p1.getName()<<") stacksize("<<game.getP1()->stacksize()<<")"<<endl;
   for (int i=0;i<5;i++) {
    game.playTurn();
   }
